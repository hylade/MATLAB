...
即 $\frac{d}{dx}(F-y'F_{y'})=0$ ，那么 $F-y'F_{y'}=c$

若将 F 代入，那么 $\sqrt{\frac{1+y'^2}{y}}-\frac{y'^2}{\sqrt{y(1+y'^2)}}=c$

可以猜测 $y(1+y'^2)=1/c^2$ ，那么 满足的条件可以为

$\begin{cases}x=c_1(t-\sin t)+c_2\\y=c_1(1-\cos t)  \end{cases}$ ，即圆滚线方程
...
生产任务是在一定时间内提供一定数量的产品

生产费用随着生产率（单位时间的产量）的增加而变大

贮存费用随着已经生产出来的产量的增加而变大
...
寻求最优生产计划，使完成生产任务所需的总费用（生产费用和贮存费用之和）最小



#### 分析与假设

生产任务： t=0 开始生产，t=T 提供数量为 Q 的产品

生产计划（累积产量）：x(t) ；生产率（单位时间产量）： $\dot{x}(t)$

生产费用 $f(\dot{x}(t))$ ； 贮存费用 g(x(t))

故总费用为 $C(x(t))=\int_0^T[f(\dot{x}(t))+g(x(t))]dt$ 

由于生产率提高一个单位的生产费用与生产率成正比

$\frac{df}{d\dot{x}}\varpropto\dot{x}$ ，故 $f(\dot{x}(t))=k_1\dot{x}^2(t)$

贮存费用与贮存量成正比，故 $g(x(t))=k_2x(t)$
...
根据欧拉方程， $F=k_1\dot{x}^2(t)+k_2x(t)$ ，且 $F_x-\frac{d}{dt}F_x=0$

故 $k_2-2k_1\ddot{x}(t)=0$ ,且 $x(0)=0,x(T)=Q$

此时可以解得 $x(t)=\frac{k_2}{4k_1}t^2+\frac{4k_1Q-k_2T^2}{4k_1T}t$ 

由于需要满足 $x(t)\geq 0$ ，那么只需 $x(0)=0$ 且 $\dot{x}(0)\geq0$ 即可

此时 $Q\geq k_2/4k_1$ 即可
...
将赛程分为若干阶段，根据赛跑运动员的生理条件对各阶段的速度作最恰当的安排，以期获得最好的成绩

寻求速度安排的最佳策略是复杂的生理力学问题

Keller 提出一个简单模型，根据 4 个生理参数从最优控制的角度确定各阶段的速度函数



运动员在赛跑中要克服体内外的阻力以达到和保持一定的速度，需要发挥向前的冲力

为冲力做功提供能量的来源：赛跑前贮存在体内的能量，赛跑中通过氧的代谢作用产生的能量

这些能量怎样分配到赛跑的各个阶段，并在达到终点前将其全部用完



模型要确定的 3 个关系：冲力与速度，冲力做功与能量来源，速度与比赛成绩

将最佳成绩归结为以距离为目标与速度



赛跑中体内外的阻力与速度成正比，比例系数 $ \tau^{-1}$ 

赛跑前存贮在体内供赛拍的能量是常数 $E_0$ 

赛跑中在氧的代谢下单位时间产生得能量是常数 $\sigma$ 

运动员能发挥的最大冲力是 F

运动员具有单位质量，初速为零

比赛成绩： "一定距离下时间最短"等价为"一定时间内距离最大"



#### 一般模型

运动员赛跑速度 v(t) ，体内能量 E(t)

阻力与速度成正比，比例系数 $ \tau^{-1}$ , $\dot{v}+v/\tau=f(t)$ 

单位质量运动员，初速为零 $v(0)=0$ 

运动员的最大冲力是 F， $0\leq f(t)\leq F $ 

单位时间产生的能量 $\sigma$ ， $\dot{E}(t)=\sigma-fv$ 

赛跑前贮存的能量是 $E_0$ ， $E(0)=E_0$ ， $E(t)>0$ 

以速度 v(t) 在时间 T内跑完赛程 D ，$D(v(t))=\int_0^Tv(t)dt$

将 D 固定，求 v(t) 使 T 最小转化为 T 固定，求 v(t) 使 D 最大
...
最长的短跑赛程以体内能量 E(t) 不小于零为标准

通过 $\dot{v}+v/\tau=f(t)=F$ 可求得 $v(t)=F\tau(1-e^{-1/\tau})$ ，故可以求得 $\dot{E}(t)=\sigma-fv=\sigma-F^2\tau(1-e^{-1/\tau})$  

由于 $E(0)=E_0$ ，所以 $E(t)=E_0-(F^2\tau-\sigma)t-F^2\tau^2(1-e^{-1/\tau})$ 

当 v 小时 $t\leq t_c$ ，此时 $\sigma \geq Fv$ ，所以 E 增加

当 v 大时 $t\geq t_c$ ，此时 $\sigma < Fv$ ，所以 E 减小

所以存在 $t_c=t|_{E(t)=0}$ 

此时最远距离为  $D_c=\int_0^{t_c}v(t)dt=F\tau^2(e^{-t_c/\tau}+t_c/\tau)$ 



#### 中长跑模型

当赛程超过 $D_c$ 时，不能用最大冲力跑全程

将赛程分为 3 个阶段

初始阶段 $(0\leq t \leq t_1)$ ，用最大冲力跑，在短时间内获得高速度

中间阶段 $t_1 \leq t\leq t_2$ ，保持匀速

最后阶段 $t_2\leq t\leq T$ 把体内能量全部用完，靠惯性冲刺



问题：确定 t1 和 t2 及 3 个阶段的速度



初始阶段用最大冲力跑，与短跑模型相同， $v_1(t)=F\tau(1-e^{-1/\tau}),0\leq t\leq t_1$ ,t1 待定

中间阶段保持匀速 $v_2(t)=v_2,t_1\leq t\leq t_2$ ，t2 和 v2 待定

最后阶段由于能量用完 $E(t)=0$ ，故 $\frac{1}{2}\frac{d}{dt}v_3^2+\frac{v_3^2}{\tau}=\sigma$ ，其中 $v_3(t_2)=v_2$

故可求得 $v_3(t)=[(v_2^2-\sigma \tau)e^{-2(t-t_2)/\tau}+\sigma\tau]^{1/2},t_2\leq t \leq T$ 
...
假设在放入衣物和洗涤剂后洗衣机的运行过程为：加水-漂水-脱水-加水-漂水... 。请为洗衣机设计一种程序（包括运行多少轮，每轮加多少水等），使得在满足一定洗涤效果的条件下，总用水量最少。选用合理的数据进行计算。对照目前 常用的洗衣机的运行情况，对你的模型和结果作出评价
...
2. 每次洗漂加水量不能低于 L ，否则洗衣机无法转动；加水量不能高于 H ，否则会溢出，设 L < H
3. 每次漂洗时间足够，以便衣服上赃物充分溶入水中，从而使每次所加的水充分利用
4. 脱水时间足够，以使脏水充分脱出，即让衣服所含的脏水量达到一个极限，设这个极限为一个大于 0 的常数 C ，并由于脱水时不另加水，故 C < L



#### 假设和定义-变量定义

1. 设共进行 n 轮"洗漂-脱水"的过程，依次为第 0 轮... 第 n-1 轮
2. 第 k 轮的用水量为 $u_k$
3. 衣服上的初始赃物为 $x_0$ ，在第 k 轮脱水后的赃物量为 $x_{k+1}$



#### 建立模型-溶解特性和动态方程

在第 k 轮洗漂之后和脱水之前，赃物量 $x_k$ 已经变为 $x_k=p_k+q_k$

其中 $p_k$ 表示已溶入水中的赃物量， $q_k$ 表示尚未溶入的

$p_k$ 与第 k 轮的加水量有关， $u_k$ 越大， $p_k$ 越大

当 $u_k=L$ 时， $p_k$ 最小（=0， 洗衣机可能无法转动）

当 $u_k=H$ 时， $p_k$ 最大（假设 $p_k=Qu_k$ ，Q 为溶解率）



考虑线性关系，故 $p_k=Qx_k\frac{u_k-L}{H-L}，0<Q<1$  



第 k 轮脱水后，衣服上尚有赃物 $q_k=x_k-p_k$ 

其中脏水 C 中含有赃物量为 $C\frac{p_k}{u_k}$ 

故第 k 轮完成后，衣服上尚存的赃物总量为 $x_{k+1}=(x_k-p_k)+C\frac{p_k}{u_k}$ 

将 $p_k$ 代入得 $x_{k+1}=x_k[1-Q(1-\frac{C}{u_k})\frac{u_k-L}{H-L}]$ 



#### 优化模型

$x_n$ 是洗衣全过程结束后衣服上残存赃物量，而 $x_0$ 是初始赃物量，故 $x_n/x_0$ 反映了洗净效果

可知 $$\frac{x_n}{x_0}=\Pi_{k=1}^{n-1}[1-Q(1-\frac{C}{u_k})\frac{u_k-L}{H-L}]$$ 

总用水量为 $\sum\limits_{k=0}^{n-1}u_k$ 

故优化模型为 $\min f(u) =\sum\limits_{k=0}^{n-1}u_k$ 

且 $\Pi_{k=1}^{n-1}[1-Q(1-\frac{C}{u_k})\frac{u_k-L}{H-L}] \leq \epsilon (0<\epsilon<1)$

同时 $L\leq u_k\leq H​$ 



若令 $v_k=\frac{u_k-L}{H-L}$ ，故 $u_k=(H-L)v_k+L$ 

此时优化模型变为 $\min f(v) =\sum\limits_{k=0}^{n-1}v_k$   

$\Pi_{k=1}^{n-1}[1-Qv_k+\frac{Qv_k}{Av_k+B}] \leq \epsilon (0<\epsilon<1)$ 

其中 $A=\frac{H-L}{C}=B(\frac{H}{L}-1)$ ，$B=\frac{L}{C}$ 
